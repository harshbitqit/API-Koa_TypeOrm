...
console.log((ctx as any).params)
  let movieid = (ctx as any).params.movie_id;
  // Find the requested movie.
...
router.delete('/:movie_id', async (ctx: Context) => {
  // Get the movie repository from TypeORM.
  const movieRepo: Repository<movieEntity> = getRepository(movieEntity);
...
router.patch('/:movie_id', async (ctx: Koa.Context) => {
  ...
  let movie_id=(ctx as any).params.movie_id;
  // Find the requested movie.
  const movie: movieEntity = await movieRepo.findOne({where : {id:movie_id}});
